...
import { putMemberProfileAPI } from '../profile/services/profile'
...
const getMemberAddressById = async () => {
  if (query.id) {
    const ref = await getMemberAddressByIdAPI(query.id)
    form.value = ref.result as any
  }
}
...
<template>
  ...
  <!-- 提交按钮 -->
  <button class="button" @tap="onSubimt">保存并使用</button>
</template>
